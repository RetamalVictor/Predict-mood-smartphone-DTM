...
def model_builder(hp):
    ...
    x = keras.layers.GRU(
        units=hp_units, recurrent_dropout=0.5, return_sequences=True, unroll=True
    )(inputs)
...
callbacks = [
    keras.callbacks.ModelCheckpoint("GRU_tune_allusers.keras", save_best_only=True),
    keras.callbacks.EarlyStopping(monitor="val_loss", patience=10),
]

tuner.search(
    train_dataset, epochs=10, validation_data=validation_dataset, callbacks=callbacks
)
